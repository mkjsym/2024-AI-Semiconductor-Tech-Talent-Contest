...
def draw_bbox_with_tracking(img: np.ndarray, tracking_info: List[dict]) -> np.ndarray:
   """바운딩 박스와 global_id를 그리는 함수"""
   for track in tracking_info:
       box = track['bbox']
       global_id = track['global_id']
       color = COLORS[global_id % len(COLORS)]
       img = plot_onx_box(box, img, color)
   return img
...
def draw_bbox(
    img: np.ndarray, predictions: np.ndarray) -> np.ndarray:
    ...
    for prediction in predictions:
        ...
        img = plot_onx_box(mbox, img, color)
